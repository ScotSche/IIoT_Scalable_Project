...
class AutonomousRobotSpec extends AnyWordSpec with Matchers {
  "An autonomous robot" when {
    "new initialized" should {
      val target: AutonomousRobot = new AutonomousRobot("Robot Autonomous",
        RobotPosition(0, 0, null), RobotPosition(100, 100, null), false, false)
      ...
      "have an indicator if moving down" in {
        target.reverseMovement should be(false)
      }
